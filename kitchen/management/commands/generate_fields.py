...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        to_fill = Recipe.objects.filter(description__isnull=True) | Recipe.objects.filter(
        ...
        for recipe in to_fill:
            try:
                ...
                if result:
                    ...
                    recipe.save()
                    self.stdout.write(
                        self.style.SUCCESS(f"Updated recipe '{recipe.name}' with generated fields.")
                    )
                    # print(repr(result))
                    # print(f"Generated fields for recipe '{recipe.name}': {result.get('description')},
