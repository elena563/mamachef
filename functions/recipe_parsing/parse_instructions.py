...
from .constants import MAX_STEP_CHARS, MAX_STEPS, MIN_STEP_LEN
...
def _try_single_newline(text):
    ...
    substantial = [line.strip() for line in lines if len(line.strip()) > MIN_STEP_LEN]
...
def parse_steps(instructions):
    ...
    steps = (
        _try_numbered(text) or _try_double_newline(text) or _try_single_newline(text) or _try_sentences(text)
    )
